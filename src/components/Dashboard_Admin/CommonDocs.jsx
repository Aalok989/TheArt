...
const CommonDocs = () => {
  ...
  useEffect(() => {
    const getDocuments = async () => {
      try {
        setLoading(true);
        const response = await fetchCommonDocuments();
        if (response.success) {
          setDocuments(response.data);
        }
      } catch (error) {
        console.error('Error fetching common documents:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="flex items-center gap-3">
          ...
          <p className="text-gray-600">Loading common documents...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto min-h-0" style={{ paddingRight: 'clamp(0.25rem, 0.5rem, 1rem)' }}>
        ...
        {documents.length === 0 && (
          <div className="text-center" style={{ paddingTop: 'clamp(1.5rem, 2rem, 2.5rem)', paddingBottom: 'clamp(1.5rem, 2rem, 2.5rem)' }}>
            <p className="text-gray-500 font-montserrat" style={{ fontSize: 'clamp(0.75rem, 0.875rem, 1rem)' }}>No common documents found.</p>
          </div>
        )}
      </div>
