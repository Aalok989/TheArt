...
import { fetchNoDiscount } from '../../api/mockData';

const NoDiscount = ({ onPageChange }) => {
  const [loading, setLoading] = useState(true);
  const [noDiscountData, setNoDiscountData] = useState([]);
  ...
  useEffect(() => {
    const getNoDiscount = async () => {
      try {
        setLoading(true);
        const response = await fetchNoDiscount();
        if (response.success) {
          setNoDiscountData(response.data || []);
        }
      } catch (error) {
        console.error('Error fetching no discount:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleExportPDF = () => {
    ...
    w.document.write(`<!doctype html><html><head><title>No Discount</title>
      <style>
  ...
  const handleFlatClick = (flat) => {
    ...
    try { sessionStorage.setItem('flatOrigin', 'noDiscount'); } catch {}
    if (onPageChange) {
      onPageChange('flat');
    }
  };
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden w-full shadow-sm lg:shadow-md border lg:border-gray-200" style={{ borderRadius:'clamp(1rem,1.5rem,2rem)' }}>
      <div className="flex-shrink-0" style={{ padding:'clamp(1rem,1.5rem,2rem)', paddingBottom:'clamp(0.5rem,0.75rem,1rem)' }}>
        <div className="flex items-center gap-3 flex-wrap">
          <h2 className="font-bold text-gray-800" style={{ fontSize:'clamp(1rem,1.25rem,1.5rem)' }}>No Discount</h2>
