...
const CompletePayment = ({ onPageChange }) => {
  ...
  useEffect(() => {
    const getCompletePayment = async () => {
      try {
        setLoading(true);
  ...
  const displayed = useMemo(() => {
    ...
    if (selectedFilter === 'CLP') {
      // Assuming payment plan field exists - if not, we'll need to check mockData
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = 'complete-payment.csv';
  ...
  const handleExportPDF = () => {
    ...
    w.document.write(`<!doctype html><html><head><title>Complete Payment</title>
      <style>
  ...
  const handleFlatClick = (payment) => {
    ...
    try { sessionStorage.setItem('flatOrigin', 'completePayment'); } catch {}
    if (onPageChange) {
      onPageChange('flat');
    }
  };
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden w-full shadow-sm lg:shadow-md border lg:border-gray-200" style={{ borderRadius:'clamp(1rem,1.5rem,2rem)' }}>
      <div className="flex-shrink-0" style={{ padding:'clamp(1rem,1.5rem,2rem)', paddingBottom:'clamp(0.5rem,0.75rem,1rem)' }}>
        <div className="flex items-center gap-3 flex-wrap">
          <h2 className="font-bold text-gray-800" style={{ fontSize:'clamp(1rem,1.25rem,1.5rem)' }}>Complete Payment</h2>
