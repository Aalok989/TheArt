...
const FlatDocs = ({ onPageChange }) => {
  ...
  const [hasSelectedFlat, setHasSelectedFlat] = useState(viewFlatNoFromStorage && viewFromFlatFromStorage === 'true' ? true : false); // Track if flat has been selected
  useEffect(() => {
    const getDocuments = async () => {
      try {
        setLoading(true);
        const response = await fetchFlatDocuments();
        if (response.success) {
          ...
          if (viewFlatNo && viewFromFlat === 'true') {
            ...
            console.log('FlatDocs - Filtered documents immediately:', { block: viewBlock, flatNo: viewFlatNo, docCount: filteredDocs.length, constructedFlatNo: flatNo });
          } else {
            setDocuments(response.data);
          }
        }
      } catch (error) {
        console.error('Error fetching flat documents:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      console.log('FlatDocs - Checking view flags after load:', { viewFlatNo, viewBlock, viewFromFlat, loading, allDocumentsLength: allDocuments.length });
      
      if (viewFlatNo && viewFromFlat === 'true' && allDocuments && allDocuments.length > 0) {
        ...
        console.log('FlatDocs - Auto-selected flat and showing documents:', { block: viewBlock, flatNo: viewFlatNo, docCount: filteredDocs.length, constructedFlatNo: flatNo });
      }
    }, 150);
  ...
  // Separate useEffect to check for upload flat number from Flat page - runs after loading
  useEffect(() => {
    if (loading) return; // Wait for loading to complete
    
    // Use a small delay to ensure component is fully rendered
    const timer = setTimeout(() => {
      // Check for upload flat number from Flat page
      const uploadFlatNo = sessionStorage.getItem('uploadFlatNo');
      const uploadBlock = sessionStorage.getItem('uploadBlock');
      const uploadFromFlat = sessionStorage.getItem('uploadFromFlat');
      
      console.log('FlatDocs - Checking upload flags after load:', { uploadFlatNo, uploadBlock, uploadFromFlat, loading });
      
      if (uploadFlatNo && uploadFromFlat === 'true') {
        // Create flat info object for the upload popup (same format as FlatSelectionPopup)
        const flatInfo = {
          block: uploadBlock || 'A',
          flatNo: uploadFlatNo
        };
        
        console.log('FlatDocs - Opening upload popup for flat:', flatInfo);
        
        // Clear the sessionStorage flags FIRST to prevent multiple checks
        sessionStorage.removeItem('uploadFlatNo');
        sessionStorage.removeItem('uploadBlock');
        sessionStorage.removeItem('uploadFromFlat');
        
        // Set selected flat info and open upload popup directly
        console.log('FlatDocs - Setting state - About to open popup');
        setSelectedFlatInfo(flatInfo);
        setIsUploadPopupOpen(true);
        
        console.log('FlatDocs - State set - isUploadPopupOpen should be true, selectedFlatInfo:', flatInfo);
      }
    }, 150);
          ...
          <div className="bg-white border border-gray-200 h-full" style={{ borderRadius: 'clamp(0.5rem, 0.75rem, 1rem)', padding: 'clamp(0.75rem, 1rem, 1.5rem)' }}>
            <div className="h-full flex items-center justify-center">
              <div className="flex items-center gap-3">
                ...
                <p className="text-gray-600">Loading flat documents...</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ marginBottom: 'clamp(0.75rem, 1rem, 1.5rem)' }}>
        <div className="flex justify-between items-center">
          <div className="flex items-center" style={{ gap: 'clamp(0.5rem, 0.75rem, 1rem)' }}>
            ...
            <h2 className="font-bold text-gray-800" style={{ fontSize: 'clamp(1rem, 1.125rem, 1.5rem)', marginBottom: 'clamp(0.75rem, 1rem, 1.5rem)' }}>
              Flat Documents {selectedBlock && selectedFlat && `- Block ${selectedBlock} - Flat ${selectedFlat}`}
            </h2>
          </div>
        ...
        <div className="space-y-0">
          ...
          {documents.length === 0 && (
            <div className="text-center" style={{ paddingTop: 'clamp(1.5rem, 2rem, 2.5rem)', paddingBottom: 'clamp(1.5rem, 2rem, 2.5rem)' }}>
              <p className="text-gray-500 font-montserrat" style={{ fontSize: 'clamp(0.75rem, 0.875rem, 1rem)' }}>No flat documents found.</p>
            </div>
          )}
          </div>
        </div>
