...
import { fetchUnsignedBBA } from '../../api/mockData';

const UnsignedBBA = ({ onPageChange }) => {
  const [loading, setLoading] = useState(true);
  const [unsignedBBAData, setUnsignedBBAData] = useState([]);
  ...
  useEffect(() => {
    const getUnsignedBBA = async () => {
      try {
        setLoading(true);
        const response = await fetchUnsignedBBA();
        if (response.success) {
          setUnsignedBBAData(response.data || []);
        }
      } catch (error) {
        console.error('Error fetching unsigned BBA:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleExportPDF = () => {
    ...
    w.document.write(`<!doctype html><html><head><title>Unsigned BBA</title>
      <style>
  ...
  const handleFlatClick = (flat) => {
    ...
    try { sessionStorage.setItem('flatOrigin', 'unsignedBBA'); } catch {}
    if (onPageChange) {
      onPageChange('flat');
    }
  };
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden w-full shadow-sm lg:shadow-md border lg:border-gray-200" style={{ borderRadius:'clamp(1rem,1.5rem,2rem)' }}>
      <div className="flex-shrink-0" style={{ padding:'clamp(1rem,1.5rem,2rem)', paddingBottom:'clamp(0.5rem,0.75rem,1rem)' }}>
        <div className="flex items-center gap-3 flex-wrap">
          <h2 className="font-bold text-gray-800" style={{ fontSize:'clamp(1rem,1.25rem,1.5rem)' }}>Unsigned BBA</h2>
