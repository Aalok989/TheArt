...
import { fetchExtraPayment } from '../../api/mockData';

const ExtraPayment = ({ onPageChange }) => {
  ...
  useEffect(() => {
    const getExtraPayment = async () => {
      try {
        setLoading(true);
        const response = await fetchExtraPayment();
        if (response.success) {
          setExtraPaymentData(response.data || []);
        }
      } catch (error) {
  ...
  const displayedRows = useMemo(() => {
    ...
    if (q) {
      filtered = filtered.filter(item =>
        ...
        item.dueAmount?.includes(q) ||
        item.paidAmount?.includes(q)
      );
    }
    
    return filtered;
  }, [extraPaymentData, searchQuery]);
  ...
  const headers = [
    ...
    { key: 'dueTax', label: 'Due Tax' },
    { key: 'paidTax', label: 'Paid Tax' },
    { key: 'pendingTax', label: 'Pending Tax' }
  ];

  const handleCopy = async () => {
    const csv = [headers.map(h => h.label).join('\t'), ...displayedRows.map(r => [
      r.srNo || '', r.flatNo || '', r.name || '', r.email || '', r.contactNo || '', 
      r.dueAmount || '', r.paidAmount || '', r.dueTax || '', r.paidTax || '', r.pendingTax || ''
    ].join('\t'))].join('\n');
    try {
  ...
  const handleExportPDF = () => {
    ...
    w.document.write(`<!doctype html><html><head><title>Extra Payment</title>
      <style>
  ...
  const handleFlatClick = (flat) => {
    ...
    try { sessionStorage.setItem('flatOrigin', 'extraPayment'); } catch {}
    if (onPageChange) {
      onPageChange('flat');
    }
  };
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden w-full shadow-sm lg:shadow-md border lg:border-gray-200" style={{ borderRadius:'clamp(1rem,1.5rem,2rem)' }}>
      <div className="flex-shrink-0" style={{ padding:'clamp(1rem,1.5rem,2rem)', paddingBottom:'clamp(0.5rem,0.75rem,1rem)' }}>
        <div className="flex items-center gap-3 flex-wrap">
          <h2 className="font-bold text-gray-800" style={{ fontSize:'clamp(1rem,1.25rem,1.5rem)' }}>Extra Payment</h2>
              ...
              {paginatedRows.length === 0 ? (
                ...
                paginatedRows.map((item, idx) => (
                  <tr key={idx} className="bg-white even:bg-gray-50">
                    ...
                    <td className="border border-gray-200 px-3 py-2">{item.dueAmount}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.paidAmount}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.dueTax}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.paidTax}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.pendingTax}</td>
                  </tr>
                ))
              )}
            </tbody>
