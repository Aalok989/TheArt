...
const STEP_META = {
  project: {
    ...
    subtitle: 'Choose the project the staff will belong to.'
  },
  account: {
    title: 'Account & Personal Info',
    subtitle: 'Provide login and personal details.'
  },
  employment: {
    title: 'Employment & Permissions',
    subtitle: 'Fill role, salary, status, and access controls.'
  }
};

const ROLE_OPTIONS = ['MANAGER', 'EXECUTIVE', 'SUPPORT', 'FINANCE', 'ADMIN'];
const STATUS_OPTIONS = ['ACTIVE', 'INACTIVE', 'SUSPENDED'];

const AddStaff = ({ onPageChange, onSuccess }) => {
  ...
  const stepSequence = ['project', 'account', 'employment'];
  ...
  const validateStep = (stepId) => {
    ...
    if (stepId === 'account') {
      ...
      if (!staffForm.email.trim() || !validateEmail(staffForm.email)) newErrors.email = 'Valid email is required';
      if (!staffForm.password.trim() || staffForm.password.length < 6) newErrors.password = 'Password must be at least 6 chars';
      if (!staffForm.firstName.trim()) newErrors.firstName = 'First name is required';
      if (!staffForm.lastName.trim()) newErrors.lastName = 'Last name is required';
      if (!staffForm.phoneNumber.trim() || !/^\d{10}$/.test(staffForm.phoneNumber.trim())) newErrors.phoneNumber = 'Phone must be 10 digits';
      if (!staffForm.address.trim()) newErrors.address = 'Address is required';
    }
    if (stepId === 'employment') {
      if (!staffForm.role.trim()) newErrors.role = 'Role is required';
      if (!staffForm.department.trim()) newErrors.department = 'Department is required';
      if (!staffForm.employeeId.trim()) newErrors.employeeId = 'Employee ID is required';
      if (!staffForm.joiningDate.trim()) newErrors.joiningDate = 'Joining date is required';
      if (!staffForm.salary.trim() || isNaN(Number(staffForm.salary))) newErrors.salary = 'Salary must be numeric';
      if (!staffForm.status.trim()) newErrors.status = 'Status is required';
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async () => {
    if (!validateStep('employment')) {
      setCurrentStep(stepSequence.indexOf('employment') + 1);
      return;
    }
    try {
      setSubmitting(true);
      const response = await accountsAPI.createStaff(selectedProject?.id, staffForm);
      ...
      <div className="w-full lg:w-[30%] min-w-0 flex flex-col max-h-[50%] lg:max-h-none overflow-hidden">
        ...
        >
          ...
          <p className="text-gray-600 break-words" style={{ fontSize: 'clamp(0.75rem, 0.875rem, 1rem)' }}>
            Onboard a staff user to the selected project.
          </p>
        </div>
        <div
      ...
      <div className="w-full lg:w-[70%] min-w-0 bg-[#F3F3F3FE] border-t lg:border-t-0 lg:border-l border-gray-300 flex flex-col flex-1 lg:flex-none overflow-hidden">
        ...
        >
          <h2 className="font-bold text-gray-800" style={{ fontSize: 'clamp(0.875rem, 1rem, 1.125rem)' }}>
            {steps.find(step => step.active)?.title || 'Add Staff'}
          </h2>
        </div>
        <div
          className="flex-1 overflow-auto min-h-0"
                ...
                {currentStepId === 'account' && (
                  <>
                    <div className="mb-6 pb-4 border-b border-gray-200">
                      ...
                      <p className="text-gray-500 text-sm">Enter login credentials and personal details</p>
                    </div>
                    <div className="space-y-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="staff3@example.com"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-1 col-span-1">
                          ...
                          <textarea
                            ...
                            rows={2}
                            placeholder="Office Tower, Sector 42"
                ...
                {currentStepId === 'employment' && (
                  <>
                    <div className="mb-6 pb-4 border-b border-gray-200">
                      ...
                      <p className="text-gray-500 text-sm">Provide role, salary and access controls</p>
                    </div>
                    <div className="space-y-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="relative">
                            <select
                              ...
                              className={`w-full border ${errors.role ? 'border-red-400' : 'border-gray-300'} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent appearance-none`}
                            >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="Sales"
                            className={`w-full border ${errors.department ? 'border-red-400' : 'border-gray-300'} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent`}
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="EMP-001"
                            className={`w-full border ${errors.employeeId ? 'border-red-400' : 'border-gray-300'} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent`}
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full border ${errors.joiningDate ? 'border-red-400' : 'border-gray-300'} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent`}
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="650000.00"
                            className={`w-full border ${errors.salary ? 'border-red-400' : 'border-gray-300'} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent`}
                          />
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <select
                              ...
                              className={`w-full border ${errors.status ? 'border-red-400' : 'border-gray-300'} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent appearance-none`}
                            >
                          ...
                          >
                            <span
                              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                                staffForm.isActive ? 'translate-x-5' : 'translate-x-1'
                              }`}
                            />
                          </button>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" checked={staffForm.canViewCustomers} onChange={() => handleToggle('canViewCustomers')} />
                            View Customers
                          </label>
                          <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" checked={staffForm.canEditCustomers} onChange={() => handleToggle('canEditCustomers')} />
                            Edit Customers
                          </label>
                          <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" checked={staffForm.canViewFinancials} onChange={() => handleToggle('canViewFinancials')} />
                            View Financials
                          </label>
                          <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" checked={staffForm.canEditFinancials} onChange={() => handleToggle('canEditFinancials')} />
                            Edit Financials
                          </label>
                          <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" checked={staffForm.canViewReports} onChange={() => handleToggle('canViewReports')} />
                            View Reports
                          </label>
                          <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" checked={staffForm.canManageProperties} onChange={() => handleToggle('canManageProperties')} />
                            Manage Properties
                          </label>
                        </div>
                      </div>
