...
import { fetchExtraDiscount } from '../../api/mockData';

const ExtraDiscount = ({ onPageChange }) => {
  const [loading, setLoading] = useState(true);
  const [extraDiscountData, setExtraDiscountData] = useState([]);
  ...
  useEffect(() => {
    const getExtraDiscount = async () => {
      try {
        setLoading(true);
        const response = await fetchExtraDiscount();
        if (response.success) {
          setExtraDiscountData(response.data || []);
        }
      } catch (error) {
        console.error('Error fetching extra discount:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const displayedRows = useMemo(() => {
    let filtered = extraDiscountData || [];
    ...
    if (q) {
      filtered = filtered.filter(item =>
        ...
        item.paymentPlan?.toLowerCase().includes(q) ||
        item.dealer?.toLowerCase().includes(q)
      );
    }
    
    return filtered;
  }, [extraDiscountData, searchQuery]);
  ...
  const headers = [
    ...
    { key: 'companyRate', label: 'Company Rate' },
  ...
  const handleCopy = async () => {
    const csv = [headers.map(h => h.label).join('\t'), ...displayedRows.map(r => [
      r.srNo || '', r.flatNo || '', r.name || '', r.email || '', r.contactNo || '', 
      r.paymentPlan || '', r.dealer || '', r.companyRate || '', r.loginRate || '', r.bookingDate || ''
    ].join('\t'))].join('\n');
    try {
  ...
  const handleExportPDF = () => {
    ...
    w.document.write(`<!doctype html><html><head><title>Discounted Flat</title>
      <style>
  ...
  const handleFlatClick = (flat) => {
    ...
    try { sessionStorage.setItem('flatOrigin', 'extraDiscount'); } catch {}
    if (onPageChange) {
      onPageChange('flat');
    }
  };
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden w-full shadow-sm lg:shadow-md border lg:border-gray-200" style={{ borderRadius:'clamp(1rem,1.5rem,2rem)' }}>
      <div className="flex-shrink-0" style={{ padding:'clamp(1rem,1.5rem,2rem)', paddingBottom:'clamp(0.5rem,0.75rem,1rem)' }}>
        <div className="flex items-center gap-3 flex-wrap">
          <h2 className="font-bold text-gray-800" style={{ fontSize:'clamp(1rem,1.25rem,1.5rem)' }}>Discounted Flat</h2>
              ...
              {paginatedRows.length === 0 ? (
                ...
                paginatedRows.map((item, idx) => (
                  <tr key={idx} className="bg-white even:bg-gray-50">
                    ...
                    <td className="border border-gray-200 px-3 py-2">{item.email}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.contactNo}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.paymentPlan}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.dealer}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.companyRate}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.loginRate}</td>
                    <td className="border border-gray-200 px-3 py-2">{item.bookingDate}</td>
                  </tr>
                ))
              )}
            </tbody>
